...
import com.busanit501.shoesproject.service.mjsservice.ReplyService;
...
//        Map<String,Long> resultMap = Map.of("rno",123L);
    ...
    @GetMapping(value = "/shoes/{itemId}")
    public PageResponseDTO<ReplyDTO> getList(
            // 경로의 변수를 서버에 재할당.
            // 게시글 번호를 가지고 올 예정.
        @PathVariable("itemId") Long itemId, PageRequestDTO pageRequestDTO
    ) {
        PageResponseDTO<ReplyDTO> responseDTO = replyService.getListOfBoard(itemId, pageRequestDTO);
        return  responseDTO;
    } //getList
